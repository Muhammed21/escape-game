...
export const Participant = () => {
  ...
  return (
    <Container className="relative w-full h-max flex items-center justify-between">
      ...
      />
      <div className="hidden md:flex items-center w-full h-max justify-center gap-5 rotate-[-90deg]">
        ...
        {participants.map((participant, index) => (
          <div key={index} className="flex items-center justify-center gap-8">
            ...
            <Image
              src={participant.icon}
              alt={participant.nom}
              width={(participant.iconSize) / 2 || 50}
              height={(participant.iconSize) / 2 || 50}
